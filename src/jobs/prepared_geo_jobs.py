...
def _candidates(sess: Session, limit: int) -> list[tuple[int, str]]:
    """Activities with a polyline and no current-version prepared row.

    Oldest first, so a backlog drains in a stable order and a row that keeps
    failing to prepare cannot starve the rest by being retried first every time.
    """
...
def sweep_unprepared_geometry(limit: int = MAX_PREPARE_PER_SWEEP) -> int:
    ...
    try:
        with Session(engine) as sess:
            candidates = _candidates(sess, limit)
    ...
    if prepared:
        with Session(engine) as sess:
            ...
            ).bindparams(version=PREPARED_GEO_VERSION)).one()
        _log.info("prepared geometry for %d activities, %s left",
                  prepared, remaining[0] if isinstance(remaining, tuple) else remaining)
    return prepared
