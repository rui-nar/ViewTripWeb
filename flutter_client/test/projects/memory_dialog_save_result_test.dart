...
// memory-add flow:
//
//  - Fix 3: MemoryDialog._save() used to call createMemory and pop the
//    dialog unconditionally, even though createMemory swallowed its own
//    exceptions. A failed create looked successful to the user while a
//    phantom optimistic item lingered. createMemory now returns a bool the
...
//    which this follows).
//  - Fix 4: uploadMemoryPhoto's return value (null on failure) was never
...
import 'package:viewtrip_client/src/projects/memory_dialog.dart';
...
// Opens MemoryDialog via a real showDialog(), like the app does — pushing an
// actual second route so Navigator.of(context).pop() inside _save() closes
// just the dialog, not the whole screen (and its ScaffoldMessenger, which
// the photo-upload-failure test below needs to survive the save).
...
void main() {
  ...
  testWidgets(
    ...
    // uploadMemoryPhoto returns null immediately — a deterministic stand-in
